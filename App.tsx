        ...
        {mobileMenuOpen && (
          <div className="md:hidden bg-slate-900 border-b border-slate-800 p-4 animate-in slide-in-from-top-2">
            <div className="flex flex-col space-y-4">
              ...
              <a
                href={PROFILE.resumeUrl}
                className="bg-indigo-600 text-white p-3 rounded-lg text-center font-medium"
              >
                Download Resume
              </a>
            </div>
        ...
        <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-12 items-center">
          <div className="space-y-8">
            ...
            <div className="flex flex-wrap gap-4">
              <a
                href="#projects"
                className="bg-indigo-600 hover:bg-indigo-500 text-white px-8 py-3 rounded-xl font-semibold transition-all flex items-center gap-2 shadow-lg shadow-indigo-600/20"
              >
...
function renderSkills() {
  return developer.skills.map(skill => (
    <Badge key={skill}>{skill}</Badge>
      ...
      <section
        id="skills"
        className="py-20 bg-slate-900/30 border-y border-slate-900"
      >
        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6">
            <div className="space-y-4">
              ...
              <p className="text-slate-400 max-w-lg">
                A selection of my best work, ranging from complex backends to
          ...
          <div className="grid md:grid-cols-2 gap-8">
            {filteredProjects.map((project) => (
              <div
                key={project.id}
                className="group relative bg-slate-900/50 border border-slate-800 rounded-3xl overflow-hidden hover:border-slate-700 transition-all"
              >
                ...
                <div className="p-8">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {project.tags.map((tag) => (
                      <span
                        key={tag}
                        className="px-3 py-1 bg-slate-800/80 border border-slate-700 text-slate-300 text-xs rounded-full font-mono"
                      >
                        {tag}
                      </span>
                    ))}
      ...
      <section id="contact" className="py-20 px-4">
        <div className="max-w-4xl mx-auto bg-gradient-to-br from-indigo-900/20 to-purple-900/20 border border-indigo-500/20 rounded-[40px] p-8 md:p-16 text-center space-y-8">
          ...
          <p className="text-xl text-slate-400">
            Currently open to full-time roles and high-impact freelance
            projects.
          </p>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 pt-4">
            <a
              href={`mailto:${PROFILE.email}`}
              className="w-full sm:w-auto bg-white text-slate-950 px-10 py-4 rounded-2xl font-bold hover:bg-slate-200 transition-all flex items-center justify-center gap-2"
            >
